...
export async function POST(request) {
  try {
    ...
    // FIX 4: Accept fullName from the request body (was referenced but never declared)
    const { courseId, userId, email, fullName, successUrl, cancelUrl } = body;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        successUrl ||
        `${process.env.NEXT_PUBLIC_APP_URL}/thank-you?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url:
        cancelUrl || `${process.env.NEXT_PUBLIC_APP_URL}/courses/${courseId}`,

      // ✅ These metadata fields are what the webhook reads to enroll the user
      metadata: {
        course_id: courseId,       // getCourseIdFromSession() reads this
        user_id: userId,           // resolveUserId() Strategy A reads this
        customer_email: email,     // resolveUserId() Strategy B fallback
        customer_name: fullName || "", // ✅ FIX 4: safe — empty string if not provided
      },
